"""
skipthecommercials.xyz scraper.

Uses the /api/user?api_token=<token> endpoint.

Required env var:
    SKIPTHECOMMERCIALS_TOKEN
"""
...
USER_STATS_URL = "https://skipthecommercials.xyz/api/user"


async def get_stats(_: bool = False) -> Dict[str, Any]:
    try:
        token = os.getenv("SKIPTHECOMMERCIALS_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing SKIPTHECOMMERCIALS_TOKEN")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get SkipTheCommercials stats: HTTP {e.code}, Reason {e.reason}")
