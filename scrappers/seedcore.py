"""
seedcore.net scraper.

Uses the /api/user?api_token=<token> endpoint.
Generate the token from your account settings page.
...
USER_STATS_URL = "https://seedcore.net/api/user"


async def get_stats(_: bool = False) -> Dict[str, Any]:
    try:
        token = os.getenv("SEEDCORE_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing SEEDCORE_TOKEN")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get SeedCore stats: HTTP {e.code}, Reason {e.reason}")
