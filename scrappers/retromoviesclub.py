"""
retro-movies.club scraper.

Uses the /api/user?api_token=<token> endpoint.

Required env var:
    RETROMOVIESCLUB_TOKEN
"""
...
USER_STATS_URL = "https://retro-movies.club/api/user"


async def get_stats(_: bool = False) -> Dict[str, Any]:
    try:
        token = os.getenv("RETROMOVIESCLUB_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing RETROMOVIESCLUB_TOKEN")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get RetroMoviesClub stats: HTTP {e.code}, Reason {e.reason}")
