...
USER_STATS_URL = "https://seedpool.org/api/user"


async def get_stats(_: bool = False) -> Dict[str, Any]:
    try:
        token = os.getenv("SEEDPOOL_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing SEEDPOOL_TOKEN")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get SeedPool stats: HTTP {e.code}, Reason {e.reason}")
