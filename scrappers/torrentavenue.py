...
USER_STATS_URL = "https://torrentavenue.online/api/user"


async def get_stats(_: bool = False) -> Dict[str, Any]:
    try:
        token = os.getenv("TORRENTAVENUE_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing TORRENTAVENUE_TOKEN")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get TorrentAvenue stats: HTTP {e.code}, Reason {e.reason}")
