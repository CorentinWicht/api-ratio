"""
yu-scene.net (YUSCENE) scraper.

YUSCENE runs UNIT3D and exposes a standard /api/user?api_token= endpoint.
...
USER_STATS_URL = "https://yu-scene.net/api/user"


async def get_stats(_: bool = False) -> Dict[str, Any]:
    try:
        token = os.getenv("YUSCENE_TOKEN")
        if not token:
            raise MissingCredentialsError("Missing YUSCENE_TOKEN")
        ...
        try:
            ...
            raise ScrappingError(f"Failed to get YUSCENE stats: HTTP {e.code}, Reason {e.reason}")
